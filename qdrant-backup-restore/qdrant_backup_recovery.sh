...
get_collections() {
  ...
  _printf "%s file updated, found %d collections!\n" "$QDRANT_COLLECTIONS_FILE" "$collection_count"
}
...
create_collection_snapshot() {
  ...
  collection_count=$(wc -l < $QDRANT_COLLECTIONS_FILE)
  source_hosts_len=${#source_hosts[@]}
  if [[ ${collection_count//[[:space:]]/} =~ ^-?[0-9]+$ ]]; then
    _collection_count=$((collection_count))
  fi

  snapshot_count=$((_collection_count * source_hosts_len))
  ...
  _printf "snapshot creation summary: %d/%d created, %d failed!\n" "$success_snapshot_count" "$snapshot_count" "$failed_snapshot_count"
...
fetch_collection_snapshot() {
  ...
  while read -r item; do
    ...
    if [[ "$snapshot_name" != *"$datetime"* ]]; then
      continue
    fi
...
# clear to avoid issues where instance and more snapshots than whats stored on s3
  if [ -f "$QDRANT_SNAPSHOTS_FILE" ]; then
      rm "$QDRANT_SNAPSHOTS_FILE"
  fi
